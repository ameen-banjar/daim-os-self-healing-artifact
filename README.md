...
- `network/daim_link_agent.py` — the event-driven self-healing agent: an
  ...
  performs the actual OVSDB/adapter I/O.
- `network/test_daim_link_agent.py` — two pure-logic unit tests, runnable with
  plain `python3` and no Mininet/OVS/OVSDB: (1) BFS path computation against
  hand-verified expected flow sets; (2) the hold-down state machine driven
  through a synthetic seven-transition flapping-link sequence, with and
  without the hold-down window, confirming it suppresses 4 of 6 subsequent
  transitions before any BFS call.
- `network/stage3_autonomous_agent.py`, `stage3_link_recovery.py`,
  `stage3_controller_switch_faults.py` — the Mininet/OVS network-level test
  harnesses that produced the raw data in `results/network/` (require a
  Linux host with Open vSwitch and Mininet installed; not runnable on the
  author's development machine for this revision, see the manuscript's
  Declaration of Generative AI Assistance and Limitations sections).
- `results/network/` — raw CSVs and reports from the network-level
  measurements (`stage3_autonomous_agent_raw.csv`: 5 repetitions of
  autonomous link-failure detection and repair; `stage3_controller_switch_faults_raw.csv`:
  scripted, non-autonomous switch/controller-failure measurements).
- `results/paper3/` — the five figures used in the manuscript, generated by
...
the hold-down-enabled and hold-down-disabled runs.
...
Regenerates all five PNGs in `results/paper3/` from `results/network/`'s raw
CSV and from a live run of the hold-down unit test's decision function.
...
autonomous link-failure condition, and `network/stage3_controller_switch_faults.py`
for the scripted switch/controller conditions. This was not re-run for this
revision; `results/network/` holds the original measured output.
