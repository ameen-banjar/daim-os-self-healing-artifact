...
- `results/network/` — raw CSVs, event logs, and reports from the
  ...
  fixed).
- `results/paper3/` — the nine figures used in the manuscript, generated by
...
- `network/topology_gen.py` — generic linear/ring/fat-tree topology generator
  (produces both a Mininet `Topo` and the matching `DAIM_TOPOLOGY_CONFIG`
  JSON), `network/bfs_scaling_benchmark.py` (pure-Python BFS scaling
  microbenchmark, no Mininet needed), `network/stage3_topology_scale.py`
  (the live multi-topology/scale/overhead harness), and
  `results/network/STAGE3_TOPOLOGY_SCALE_REPORT.md`,
...
Layer 2 step 3 of the evidence-gathering plan, closing Section 10's "multiple topologies and scale"
evidence-gate item and Section 2.5's own explicitly-flagged gap ("the 200-node bound above is a
design target, not a measured result").

**BFS scaling (`bfs_scaling_benchmark.py`).** Times the real, unmodified `bfs_path()` (2000
repetitions per size, no Mininet/OVS needed since the function is pure in-process graph search)
against a deterministic grid-graph generator from 9 to 2025 nodes, source/dest hosts at opposite grid
corners (the BFS worst case for this generator). At 196 nodes (closest measured point to 200): mean
0.133 ms, max 0.205 ms across 2000 repetitions -- confirming the cited <0.3 ms/200-node design-target
bound, measured rather than assumed. Reported honestly: the bound does not hold cleanly by 400 nodes
(max 0.586 ms), and the two largest sizes tested show occasional multi-millisecond outliers (11.4 ms,
63.0 ms) attributed to running on a shared, non-realtime host, not algorithmic super-linearity.

**Multiple topologies and scale (`stage3_topology_scale.py`).** A new shared `topology_gen.py`
generates topologies structurally different from the one diamond shape every prior live experiment in
this evidence set used, with port numbers verified to match Mininet's own assignment: `linear_10` (a
10-switch chain, no redundant path for any edge -- the case self-healing cannot succeed on),
`ring_8`/`ring_20` (an 8- and a 20-switch cycle, isolating the effect of hop count from topology
type), and `fattree_k4` (a 20-switch three-tier fat-tree, exceeding the >10-switch scale target, with
multi-path redundancy at both the aggregation and core layers). 10/10 live repetitions matched their
designed outcome exactly: `linear_10` correctly and safely reported `repair_failed` (no false-positive
success, no crash); every `ring_8`/`ring_20`/`fattree_k4` fault was correctly detected and repaired,
with the fat-tree reroute confirmed live to cross a DIFFERENT core switch (`c0_0`->`c0_1`), not a
trivial same-pod detour. Repair-action time, packet loss, agent CPU time (sampled directly from
`/proc/<agent_pid>`), and flow-mod message count all scale primarily with repair PATH LENGTH, not
topology size or monitored-interface count: `ring_20`'s 10-hop repair took ~2.4x `ring_8`'s 4-hop
repair despite the identical topology shape; `fattree_k4`'s 4-hop repair matched `ring_8`'s magnitude
despite monitoring 4x as many interfaces (64 vs 16).

Full detail, result tables, and discussion: `results/network/STAGE3_TOPOLOGY_SCALE_REPORT.md`.

No new unit tests this round (both items are pure-Python/live-network experiments, not agent-logic
changes); 33/33 existing tests still pass.
