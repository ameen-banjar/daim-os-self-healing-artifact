...
- `results/network/` — raw CSVs, event logs, and reports from the
  ...
  before/after startup-synchronization scenario; `STAGE3_HOLDDOWN_FLAPPING_REPORT.md`
  and `STAGE3_STARTUP_ALREADY_DOWN_REPORT.md` document what each measurement
  found and fixed).
- `results/paper3/` — the seven figures used in the manuscript, generated by
...
Regenerates all seven PNGs in `results/paper3/` from `results/network/`'s raw
